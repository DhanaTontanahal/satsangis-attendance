...
const CuredPatients = () => {
  ...
  useEffect(() => {
    const fetchCuredPatients = () => {
      const curedPatientsRef = database.ref("medicalcamp/curedPatients");
    ...
    return () => {
      database.ref("medicalcamp/curedPatients").off("value");
    };
  }, []);
  ...
  return (
    <Container>
      ...
      <Button onClick={toggleColumnModal}>Show/Hide Columns</Button>
      {showModal && (
        <ModalOverlay>
          <ModalContent>
      ...
      {isCardView ? (
        patients.map((patient) => (
          <Card key={patient.id}>
            {selectedColumns.patientName && <h4>{patient.patientName}</h4>}
            {selectedColumns.diseaseProblem && (
              <p>Disease/Problem: {patient.diseaseProblem}</p>
            )}
            {selectedColumns.doctorAssigned && (
              <p>Doctor: {patient.doctorAssigned}</p>
            )}
            {selectedColumns.temperature && (
              <p>Temperature: {patient.temperature} °C</p>
            )}
            {selectedColumns.bp && <p>BP: {patient.bp}</p>}
            {selectedColumns.diabetesValue && (
              <p>Diabetes Value: {patient.diabetesValue} mg/dL</p>
            )}
            {selectedColumns.weight && <p>Weight: {patient.weight} kg</p>}
            {selectedColumns.height && <p>Height: {patient.height} cm</p>}
            {selectedColumns.cause && <p>Cause: {patient.cause}</p>}
            {selectedColumns.prescription && (
              <p>Prescription: {patient.prescription}</p>
            )}
            {selectedColumns.comments && <p>Comments: {patient.comments}</p>}
          </Card>
        ))
      ) : (
        <Table>
          <thead>
            <tr>
              {selectedColumns.patientName && <th>Patient Name</th>}
              {selectedColumns.diseaseProblem && <th>Disease/Problem</th>}
              {selectedColumns.doctorAssigned && <th>Doctor Assigned</th>}
              {selectedColumns.temperature && <th>Temperature (°C)</th>}
              {selectedColumns.bp && <th>BP (mmHg)</th>}
              {selectedColumns.diabetesValue && <th>Diabetes Value (mg/dL)</th>}
              {selectedColumns.weight && <th>Weight (kg)</th>}
              {selectedColumns.height && <th>Height (cm)</th>}
              {selectedColumns.cause && <th>Cause</th>}
              {selectedColumns.prescription && <th>Prescription</th>}
              {selectedColumns.comments && <th>Comments</th>}
            </tr>
          </thead>
          <tbody>
            {patients.map((patient) => (
              <tr key={patient.id}>
                {selectedColumns.patientName && <td>{patient.patientName}</td>}
                {selectedColumns.diseaseProblem && (
                ...
                {selectedColumns.temperature && <td>{patient.temperature}</td>}
                {selectedColumns.bp && <td>{patient.bp}</td>}
                {selectedColumns.diabetesValue && (
                  <td>{patient.diabetesValue}</td>
                )}
                {selectedColumns.weight && <td>{patient.weight}</td>}
                {selectedColumns.height && <td>{patient.height}</td>}
                {selectedColumns.cause && <td>{patient.cause}</td>}
                {selectedColumns.prescription && (
                  <td>{patient.prescription}</td>
                )}
                {selectedColumns.comments && <td>{patient.comments}</td>}
              </tr>
            ))}
          </tbody>
        </Table>
